...
// Templates
const templateUrls = [
  "https://i.ibb.co/m5W8TMjj/86de3ad52a9c.jpg",
  "https://i.ibb.co/LX3qWqB3/da0dde329b3c.jpg",
  "https://i.imgur.com/Gc3Hs2Q.png",
  "https://i.imgur.com/q9ZzTkR.png"
];
const templatePath = path.join(cacheDir, "coverdp7_template.png");

async function createFallbackTemplate() {
  const width = 800;
  const height = 800;
  return await Jimp.create(width, height, 0xff69b4);
}
...
function isValidName(name) {
  ...
  return !(lower.includes('facebook') || lower === 'unknown' || lower === 'user');
}
...
async function getProperName(api, uid, Users, mentionName = null) {
  try {
    ...
    const user = info[uid];
    if (user && isValidName(user.name)) return user.name;
  } catch (error) {}
  return 'User';
}
...
module.exports.run = async ({ api, event, Users }) => {
  ...
  try {
    ...
    const mentionName = mention[0] ? event.mentions[mention[0]] : null;
    ...
    // Yahan position set ki gayi hai
    const posX = 520;
    const posY = 95;
    
    template.composite(userImage, posX, posY);

    const outputPath = path.join(cacheDir, `cover_${targetID}_${Date.now()}.png`);
